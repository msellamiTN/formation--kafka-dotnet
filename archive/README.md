...
oc create route edge m04-java-api-secure \
  --service=m04-java-api --port=8080-tcp
```
...
oc create route edge m04-dotnet-consumer-secure \
  --service=m04-dotnet-consumer --port=8080-tcp
```
...
```bash
# URLs publiques
JAVA_HOST=$(oc get route m04-java-api-secure -o jsonpath='{.spec.host}')
DOTNET_HOST=$(oc get route m04-dotnet-consumer-secure -o jsonpath='{.spec.host}')
