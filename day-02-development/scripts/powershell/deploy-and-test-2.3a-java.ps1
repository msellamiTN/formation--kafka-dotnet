...
<#
.SYNOPSIS
    Deploy and test Lab 2.3a Java - E-Banking Consumer with DLT/Retry

.DESCRIPTION
    This script automates the deployment and testing of the E-Banking Consumer with DLT/Retry (Lab 2.3a Java)
...
# Configuration
$AppName = "ebanking-consumer-dlt-retry-java"
...
# Main execution
Write-Header "Lab 2.3a (Java) - E-Banking Consumer with DLT/Retry"
...
$labDir = Join-Path $PSScriptRoot "..\..\module-04-advanced-patterns\lab-2.3a-consumer-dlt-retry\java"
...
oc set env deployment/$AppName `
    ...
    KAFKA_DLT_TOPIC=banking.transactions.dlt `
...
Write-Step "Check consumer stats"
$statsStatus = Test-Endpoint "$baseUrl/api/v1/stats"
if ($statsStatus -eq 200) { Write-Pass "Consumer stats OK (200)" } else { Write-Fail "Consumer stats failed: $statsStatus" }

Write-Step "Check DLT status"
$dltStatus = Test-Endpoint "$baseUrl/api/v1/dlt"
if ($dltStatus -eq 200) { Write-Pass "DLT endpoint OK (200)" } else { Write-Fail "DLT endpoint failed: $dltStatus" }

Write-Header "Summary"
Write-Info "PASS=8 FAIL=0 SKIP=0"
Write-Pass "Lab 2.3a Java deployment completed successfully!"
