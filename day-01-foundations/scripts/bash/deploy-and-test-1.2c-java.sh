...
APP_NAME="ebanking-producer-resilient-java"
ROUTE_NAME="ebanking-producer-resilient-java-secure"
...
header "Lab 1.2c (Java) - Deploy & Test (OpenShift S2I Binary Build)"
...
cd "$(dirname "$0")/../../module-02-producer/lab-1.2c-producer-error-handling/java"
...
step "Check metrics endpoint"
METRICS_STATUS=$(http_status "$BASE_URL/api/v1/transactions/metrics")
if [ "$METRICS_STATUS" = "200" ]; then
  pass "Metrics endpoint OK (200)"
else
  fail "Metrics endpoint failed: $METRICS_STATUS"
fi

header "STEP 4: Produce test transaction"
TX='{
...
R=$(post_json "$BASE_URL/api/v1/transactions" "$TX")
if echo "$R" | jq -e '.status' >/dev/null 2>&1; then
  pass "Transaction accepted"
  echo "$R" | jq .
else
  fail "Transaction request failed"
  echo "$R"
fi
