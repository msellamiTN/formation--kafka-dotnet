...
APP_NAME="ebanking-balance-consumer-java"
...
header "Lab 1.3b (Java) - Balance Consumer Group"
...
step "Balances endpoint"
STATUS=$(http_status "$BASE_URL/api/v1/balances")
if [ "$STATUS" = "200" ]; then pass "Balances OK"; else fail "Balances: $STATUS"; fi

step "Rebalancing endpoint"
STATUS=$(http_status "$BASE_URL/api/v1/rebalancing")
if [ "$STATUS" = "200" ]; then pass "Rebalancing OK"; else fail "Rebalancing: $STATUS"; fi
