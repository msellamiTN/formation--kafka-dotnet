...
header "Lab 1.3c (Java) - Audit Consumer (Manual Commit + DLQ)"
...
cd "$(dirname "$0")/../../module-03-consumer/lab-1.3c-consumer-manual-commit/java"
...
step "Audit log endpoint"
STATUS=$(http_status "$BASE_URL/api/v1/audit")
if [ "$STATUS" = "200" ]; then pass "Audit OK"; else fail "Audit: $STATUS"; fi

step "DLQ endpoint"
STATUS=$(http_status "$BASE_URL/api/v1/dlq")
if [ "$STATUS" = "200" ]; then pass "DLQ OK"; else fail "DLQ: $STATUS"; fi
