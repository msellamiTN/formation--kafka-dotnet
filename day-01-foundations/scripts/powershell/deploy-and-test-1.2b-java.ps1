...
$AppName = "ebanking-producer-keyed-java"
$RouteName = "ebanking-producer-keyed-java-secure"
...
Write-Header "Lab 1.2b (Java) - Deploy & Test (OpenShift S2I Binary Build)"
...
Write-Header "STEP 4: Keyed partition test"
Write-Step "Produce 2 transactions with same customerId and compare partition"
...
$r1 = Send-JsonRequest "$baseUrl/api/v1/transactions" $tx1
$r2 = Send-JsonRequest "$baseUrl/api/v1/transactions" $tx2

if ($null -ne $r1 -and $null -ne $r2 -and $r1.partition -eq $r2.partition) {
    Write-Pass "Same key mapped to same partition: $($r1.partition)"
} else {
    Write-Fail "Expected same partition for same key"
    if ($null -ne $r1) { $r1 | ConvertTo-Json -Depth 10 | Write-Host }
    if ($null -ne $r2) { $r2 | ConvertTo-Json -Depth 10 | Write-Host }
}
