# =============================================================================
# Lab 1.3c (Java): Audit Consumer (Manual Commit + DLQ) - OpenShift S2I + Test
# =============================================================================
...
Write-Header "Lab 1.3c (Java) - Audit Consumer (Manual Commit + DLQ)"
...
$labDir = Join-Path $PSScriptRoot "..\..\module-03-consumer\lab-1.3c-consumer-manual-commit\java"
...
$status = Get-HttpStatus "$baseUrl/api/v1/audit"
if ($status -eq 200) { Write-Pass "Audit OK" } else { Write-Fail "Audit: $status" }

Write-Step "DLQ endpoint"
$status = Get-HttpStatus "$baseUrl/api/v1/dlq"
if ($status -eq 200) { Write-Pass "DLQ OK" } else { Write-Fail "DLQ: $status" }
