# Day-02 Lab 2.2a - Idempotent Producer (Java) Deployment Script
# Usage: .\deploy-and-test-2.2a-java.ps1 -Token YOUR_TOKEN -Server YOUR_SERVER
...
Write-ColorOutput "  Lab 2.2a - Idempotent Producer (Java)" "Blue"
Write-ColorOutput "========================================`n" "Blue"
...
if ($LASTEXITCODE -ne 0) {
    oc new-app $LAB_NAME --name=$LAB_NAME
}
...
Write-ColorOutput "`n--- Send Idempotent Transaction ---" "Yellow"
$payload = @{
...
$result = curl.exe -sk -X POST "$BASE_URL/api/v1/transactions/idempotent" -H "Content-Type: application/json" -d $payload
Write-Host $result
if ($result -match "transactionId") {
    Write-Status "Idempotent transaction PASSED"
} else {
    Write-Error "Idempotent transaction FAILED"
}
...
Write-ColorOutput "`n========================================" "Blue"
Write-ColorOutput "  Lab 2.2a Deployment Complete!" "Green"
Write-ColorOutput "========================================`n" "Blue"
