# Day-02 Lab 2.1a - Serialization (Java) Deployment Script
# Usage: .\deploy-and-test-2.1a-java.ps1 -Token YOUR_TOKEN -Server YOUR_SERVER
...
$LAB_PATH = "day-02-development/module-04-advanced-patterns/lab-2.1a-serialization/java"
...
if (!(Get-Command curl -ErrorAction SilentlyContinue)) {
    Write-Error "curl command not found. Please install curl."
    exit 1
}
...
Write-Status "Testing batch transactions..."
$batchBody = @(
    @{
        fromAccount = "FR7630001000123456789"
        toAccount = "FR7630001000987654321"
...
try {
    $BATCH_RESPONSE = Invoke-RestMethod -Uri "$API_URL/api/v1/transactions/batch" `
        -Method POST `
        -ContentType "application/json" `
        -Body $batchBody `
        -SkipCertificateCheck
    
    if ($BATCH_RESPONSE.message -match "completed") {
        Write-Status "Batch transaction test passed"
    } else {
        Write-Error "Batch transaction test failed: $($BATCH_RESPONSE | ConvertTo-Json)"
    }
} catch {
    Write-Error "Batch transaction test failed: $($_.Exception.Message)"
}
...
Write-Status "Verifying messages in Kafka..."
Start-Sleep 5
...
try {
    $messageOutput = oc exec $KAFKA_POD -- /opt/kafka/bin/kafka-console-consumer.sh `
        --bootstrap-server localhost:9092 `
        --topic $TOPIC `
        --from-beginning `
        --max-messages 10 `
    ...
    $MESSAGE_COUNT = ($messageOutput -split "`n" | Where-Object { $_.Trim() -ne "" }).Count
    if ($MESSAGE_COUNT -gt 0) {
        Write-Status "Messages found in Kafka: $MESSAGE_COUNT messages"
    } else {
        Write-Warning "No messages found in Kafka (may need more time)"
    }
} catch {
    Write-Warning "Failed to verify messages in Kafka: $($_.Exception.Message)"
}
...
Write-Host ""
Write-ColorOutput "Test Commands:" "Yellow"
Write-Host "curl -k -X POST `"$API_URL/api/v1/transactions`" \"
Write-Host "  -H `"Content-Type: application/json`" \"
Write-Host "  -d '{`"fromAccount`":`"FR76...`",`"toAccount`":`"FR76...`",`"amount`":1000.00,`"currency`":`"EUR`",`"type`":`"TRANSFER`",`"description`":`"Test`",`"customerId`":`"CUST-001`"}'"
Write-Host ""
Write-Host "curl -k `"$API_URL/api/v1/transactions/statistics`""
Write-Host ""
