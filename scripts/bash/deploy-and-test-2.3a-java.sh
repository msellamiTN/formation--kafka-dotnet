...
# Usage: ./deploy-and-test-2.3a-java.sh --token=YOUR_TOKEN --server=YOUR_SERVER
...
echo -e "${BLUE}\n===========================================${NC}"
echo -e "${BLUE}  Lab 2.3a - Consumer DLT & Retry (Java)${NC}"
echo -e "${BLUE}===========================================\n${NC}"
...
echo -e "\n${YELLOW}--- Processed Transactions ---${NC}"
curl -sk "$BASE_URL/api/v1/processed" | python3 -m json.tool 2>/dev/null || curl -sk "$BASE_URL/api/v1/processed"

echo -e "\n${YELLOW}--- DLQ Records ---${NC}"
curl -sk "$BASE_URL/api/v1/dlq" | python3 -m json.tool 2>/dev/null || curl -sk "$BASE_URL/api/v1/dlq"

echo -e "\n${YELLOW}--- Stats ---${NC}"
curl -sk "$BASE_URL/api/v1/stats" | python3 -m json.tool 2>/dev/null || curl -sk "$BASE_URL/api/v1/stats"

echo -e "\n${BLUE}===========================================${NC}"
echo -e "${GREEN}  Lab 2.3a Deployment Complete!${NC}"
echo -e "${BLUE}===========================================\n${NC}"
