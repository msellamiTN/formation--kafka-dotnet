...
# Usage: ./deploy-and-test-2.1a-java.sh --token=YOUR_TOKEN --server=YOUR_SERVER
...
echo -e "${BLUE}🚀 Day-02 Lab 2.1a - Serialization (Java) Deployment${NC}"
echo -e "${BLUE}=====================================================${NC}"
...
oc exec "$KAFKA_POD" -- /opt/kafka/bin/kafka-topics.sh \
    ...
    --partitions 6 \
    --replication-factor 3 || {
    print_warning "Failed to create DLQ topic $DLQ_TOPIC (may already exist)"
}
...
print_status "Testing serialization endpoints..."

# Test V1 transaction
print_status "Testing V1 transaction creation..."
V1_RESPONSE=$(curl -k -s -X POST "$API_URL/api/v1/transactions" \
...
if echo "$V1_RESPONSE" | grep -q "submitted"; then
    print_status "V1 transaction test passed"
else
    print_error "V1 transaction test failed: $V1_RESPONSE"
fi

# Test V2 transaction
print_status "Testing V2 transaction creation..."
V2_RESPONSE=$(curl -k -s -X POST "$API_URL/api/v1/transactions/v2" \
    -H "Content-Type: application/json" \
    -d '{
...
if echo "$V2_RESPONSE" | grep -q "submitted"; then
    print_status "V2 transaction test passed"
else
    print_error "V2 transaction test failed: $V2_RESPONSE"
fi
...
print_status "Testing statistics endpoint..."
STATS_RESPONSE=$(curl -k -s "$API_URL/api/v1/transactions/statistics")
if echo "$STATS_RESPONSE" | grep -q "processed"; then
    print_status "Statistics endpoint test passed"
else
    print_error "Statistics endpoint test failed: $STATS_RESPONSE"
fi
...
echo -e "${GREEN}Statistics:${NC} $API_URL/api/v1/transactions/statistics"
...
echo -e "\n${BLUE}curl -k \"$API_URL/api/v1/transactions/statistics\"${NC}"
echo -e "\n${GREEN}🚀 Lab 2.1a - Serialization (Java) is ready for use!${NC}"
